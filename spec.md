...
| **Nhân lực** | Một người |
| **Phiên bản đặc tả** | 3.5 — 2026-09-18 |
...
**Thay đổi ở 3.5** — **trang Tốc độ (FR-101, FR-102).** Trần bitrate từng PID, nhịp lặp từng bảng và tốc độ cả dòng chuyển từ hằng số trong code sang `config/toc-do.yaml` sửa được trên giao diện. Trần của ETSI TS 101 211 §4.1 được ghim thành phép kiểm **chặn được**, không phải một dòng chú thích.

**Thay đổi ở 3.4** — **công tắc bật/tắt linkage của từng bouquet (FR-100).** Tắt thì giữ nguyên dữ liệu, chỉ thôi ghi descriptor vào bảng. Có vì linkage user-defined mang khối byte thô không có đặc tả (RO-8) — xoá rồi gõ lại là chuyện may rủi.

**Thay đổi ở 3.3** — **`eitinject` tự gỡ chương trình vừa kết thúc, nên FR-98 một mình không đổi được gì trên sóng (FR-99).** Đo: bảng sinh ra bắt đầu `00:00` UTC với 802 sự kiện, trên sóng bắt đầu `06:45` với 466 — 336 sự kiện không bao giờ lên sóng. Thêm `--lazy-schedule-update` để giữ tới hết phân đoạn ba giờ đang chạy. **Không** đạt được mức của Barrowa (giữ cả ngày từ `00:00`); TSDuck không có cách nào làm vậy.

**Thay đổi ở 3.2** — **cửa sổ EIT bắt đầu từ 00:00 UTC hôm nay, không phải từ bây giờ (FR-98).** EIT schedule sub-table 0x50 phủ *ngày 0–3*, ngày 0 bắt đầu nửa đêm UTC và chia thành phân đoạn ba giờ; cắt ở `now` là phát ra một ngày 0 khuyết đầu. Hai bản thu Barrowa cách nhau bốn tiếng đều bắt đầu đúng `00:00:00` UTC. Ta phát 427 sự kiện, Barrowa 705.

**Thay đổi ở 3.1** — **báo phần lịch nằm sau một khoảng trống dài (FR-97).** File lịch ngày 14/9 mang 292 sự kiện đề ngày 2 và 3 tháng 10, toàn bộ thuộc dịch vụ 838, mỗi cái lặp bốn lần, trong khi 16/9 tới 1/10 trống trơn. Bên cấp lịch xác nhận là lỗi nhập liệu. Cửa sổ 192 giờ che được hôm nay nhưng chỉ tới khoảng **24/9**; từ hôm đó rác lọt vào cửa sổ và lên sóng như lịch thật.

**Thay đổi ở 3.0** — **đổi số dịch vụ giữa sổ lịch và sổ sóng (FR-96).** Bên cấp lịch đánh số theo sổ của họ: Quảng Trị lên sóng là `825` nhưng trong file lịch là `875`. Không có chỗ khai điều đó nên `only_services` loại sạch, và kênh ấy **không có EPG nào trên sóng** — lặng lẽ. Barrowa gọi tính năng này là *Mapping*; ta gọi là `epg_source_id`.

**Thay đổi ở 2.9** — **lịch cũ đè lịch mới (FR-95).** Hai lỗi chồng nhau: hộp thư nạp file theo *tên* chứ không theo *lúc nhận*, và phép gộp định danh sự kiện bằng (dịch vụ, giờ bắt đầu) nên chương trình bị dời giờ để lại bản cũ nằm cạnh bản mới, rồi khử chồng lấn giữ đúng bản cũ. Đo trên máy phát: 290 trong 670 sự kiện lên sóng không còn trong đợt giao mới nhất; Barrowa đọc cùng nguồn chỉ lệch 31 trên 740. Sửa bằng cách đọc hai thuộc tính `start_time`/`end_time` trên `<SERVICE>` mà trình đọc vẫn vứt bỏ.
...
| **FR-52** | Tích luỹ nhiều đợt giao thành cửa sổ 192 giờ và loại sự kiện đã qua. Quy tắc ai-đè-ai ở **FR-95**: theo phạm vi `<SERVICE>` tự khai khi có, và chỉ khi không có mới lui về khử trùng lặp theo `(service_id, thời điểm bắt đầu)`. Xem RO-13. |
...
| **FR-95** | **Một đợt giao lịch thay TRỌN khoảng nó tự khai phụ trách.** Nguồn khai phạm vi ngay trên thẻ `<SERVICE start_time=… end_time=…>`; đợt nào khai thì mọi sự kiện cũ của dịch vụ đó nằm trong khoảng ấy bị gỡ trước khi đổ sự kiện mới vào. Đợt **không** khai thì chỉ được ghi đè khi trùng (dịch vụ, giờ bắt đầu) — cố ý giữ mức yếu, vì suy phạm vi ra từ chính các sự kiện là tự cho mình quyền xoá dữ liệu nguồn chưa tuyên bố phụ trách, và một file lỗi còn ba sự kiện sẽ quét sạch cả ngày. Kèm theo: hộp thư nạp theo **`st_mtime`**, không theo tên file — tên file là *ngày lịch*, không phải *ngày giao*, và trên máy phát thật `2026-09-15.xml` (nhận 08:29) đứng sau `2026-09-14.xml` (nhận 09:37) khi sắp chữ nên đợt cũ thắng đợt mới. Hai lỗi này cộng lại đưa 290/670 sự kiện lỗi thời lên sóng. |
| **FR-96** | **`epg_source_id`: số hiệu một dịch vụ mang trong file lịch, khi khác số nó mang trên sóng.** Khai trên chính dịch vụ đó trong `config/services/*.yaml`, và sửa được trên trang dịch vụ của giao diện. Để trống khi hai số trùng nhau, tức phần lớn kênh; khai trùng số thì hệ tự bỏ, vì để lại chỉ khiến người đọc sau tưởng có đổi số. Phép đổi chạy **trước** gộp và cắt (`window.remap`), và đổi cả phạm vi `<SERVICE>` tự khai chứ không riêng sự kiện — đổi muộn hơn thì `merge` coi hai đợt của cùng một kênh là hai kênh, và `drop_overlaps` không thấy chồng lấn giữa chúng, nên hai lịch của cùng một kênh cùng lên sóng. **Hai sổ có thể trùng số mà khác kênh:** SDT của ta cũng có dịch vụ `875`, là QUANG NGAI 2 RADIO, không liên quan gì tới Quảng Trị. Phép đổi tra theo số của *nguồn* nên không lẫn được; nhưng nếu ai bật EPG cho kênh radio ấy mà không khai đổi số thì lịch Quảng Trị sẽ lên sóng dưới tên kênh radio. Hôm nay không xảy ra vì kênh đó đang tắt EPG — đó là may, không phải thiết kế, nên ghi lại ở đây. |
| **FR-97** | **Báo — không tự vứt — phần lịch nằm sau một khoảng trống dài.** Ngưỡng **hai ngày**, đo trên *toàn mạng* chứ không từng kênh: một kênh nghỉ hai ngày là bình thường, cả mười tám kênh cùng không có gì trong hai ngày thì không. Chỉ xét sự kiện **chưa kết thúc**: hộp thư giữ cả lịch cũ nên quá khứ đầy khoảng trống hợp lệ, và không lọc thì cảnh báo kêu 7101 sự kiện trên 45 kênh — kêu về mọi thứ là không nói gì. Đo **trước** khi cắt cửa sổ, nếu không thì cảnh báo chỉ hiện đúng hôm rác lên sóng, muộn mất tám ngày. Không tự vứt, cùng khuôn với FR-48 và `reject_overlong`: một phép đoán không được phép lặng lẽ xoá lịch thật. Trên dữ liệu thật ngày 2026-09-15 nó chỉ ra đúng 73 sự kiện của riêng dịch vụ 838. Dịch vụ 838 cũng là nguồn của RO-22 — cùng một kênh, lỗi nguồn lần thứ hai. |
| **FR-98** | **Mép đầu cửa sổ EIT là 00:00 UTC của ngày hiện tại**, không phải thời điểm hiện tại. Giữ cả chương trình đã phát xong trong ngày. Lý do không phải là bắt chước: EIT schedule sub-table 0x50 phủ *ngày 0–3*, ngày 0 bắt đầu **nửa đêm UTC** và chia thành các phân đoạn ba giờ, nên cắt ở `now` là phát ra một ngày 0 khuyết đầu. Chỗ này người xem thấy được: đầu thu cho cuộn ngược lại chương trình đã chiếu trong ngày sẽ hiện trống ở hệ này mà đầy ở hệ kia, và VTC-SI là hệ dự phòng nên phải trùng. Đo: hai bản thu Barrowa cách nhau bốn tiếng đều bắt đầu đúng `00:00:00` UTC — **không** phải nửa đêm Hà Nội (17:00 UTC). Mép cuối vẫn tính từ `now`: nó là độ sâu **còn lại về phía trước**, neo vào đầu ngày thì cửa sổ ngắn dần đi trong ngày. Giá phải trả **nhỏ hơn dự đoán ban đầu**: file XML lớn thêm chừng 46 % (521 → 802 sự kiện), nhưng PID 18 thì gần như không đổi, vì `eitinject` lọc lại một lần nữa — xem FR-99. |
| **FR-99** | **`eitinject` gỡ ngay chương trình vừa kết thúc; dùng `--lazy-schedule-update` để giữ hết phân đoạn ba giờ.** Đo ngày 2026-09-15: bảng ta sinh ra bắt đầu `2026-09-14 23:45` với **802** sự kiện, bản thu trên sóng bắt đầu `06:45` với **466** — tức 336 sự kiện được nạp vào nhưng **không bao giờ phát**, và `06:45` chính là chương trình đang chạy lúc thu. Nên FR-98 một mình **không đổi gì ở đầu ra**; nó là *điều kiện cần*, vì cắt ở `now` thì sự kiện của phân đoạn hiện tại chưa bao giờ được nạp và cờ lazy cũng không có gì để giữ. Hai thứ phải đi cùng nhau. **Trần không vượt qua được:** theo tài liệu TSDuck, cờ này giữ tới hết phân đoạn ba giờ đang chạy, còn Barrowa giữ từ `00:00` — không có tuỳ chọn nào cho cả ngày, nên chênh lệch tối đa ba giờ là mức gần nhất đạt được mà vẫn dùng bộ sinh EIT của TSDuck. **Tác dụng thật của cờ thì chưa đo được:** nó chỉ giữ thêm chương trình **đã kết thúc** trong phân đoạn hiện tại, nên bản thu lúc `09:11` — mới qua mốc `09:00` mười một phút — không có gì để nhìn. Mép đầu `08:00` quan sát được chỉ là chương trình dài nhất còn đang chạy, tức hành vi vốn có, không phải tác dụng của cờ. Muốn đo thật thì thu vào **cuối một phân đoạn** (ví dụ `11:45`). Đồng hồ của `eitinject` thì **đã kiểm và đúng**: p/f khai đang phát `08:15–09:20` lúc thật là `09:11`. Đây cũng là chỗ **`eitinject` quên** (chương trình đã qua) trái ngược với chỗ nó **không bao giờ quên** (dịch vụ đã rút khỏi lịch, FR-94); hai hành vi khác nhau, đừng suy cái này ra cái kia. |
| **FR-100** | **Công tắc bật/tắt linkage cho từng bouquet, trên trang chủ, có hỏi lại trước khi làm.** Tắt **giữ nguyên dữ liệu** và chỉ thôi ghi `linkage_descriptor` vào BAT; bật lại là byte y nguyên. Đó là lý do nó tồn tại bên cạnh nút xoá: linkage user-defined mang khối byte thô ta không có đặc tả (RO-8), nên gõ lại được là chuyện may rủi. Hộp xác nhận **nói đúng hậu quả của chính bouquet đó**: bouquet không mang kênh nào (0x0044, 0x3622) thì tắt xong thành **bảng BAT rỗng** — chính là RO-15 — và đầu thu đang dùng đường OTA ấy mất đường nâng cấp; bouquet có kênh (0x6510) thì chỉ mất con trỏ, kênh giữ nguyên. **Không tự ý bỏ phát cả bảng** khi bouquet thành rỗng: bảng biến mất khỏi sóng là thay đổi lớn hơn nhiều so với những gì một công tắc nên tự quyết, và cảnh báo RO-15 sẵn có sẽ kêu cho người trực tự xử. **Chỉ áp dụng cho bouquet, không cho NIT**: NIT đánh version bằng tay (FR-92), nên một công tắc ở đó sẽ đổi nội dung mà không đổi version — đầu thu không nhận ra, và đó là kiểu hỏng tệ nhất. |
| **FR-101** | **Tốc độ và nhịp lặp nằm trong `config/toc-do.yaml`, sửa trên trang Tốc độ.** Ba nhóm số trả lời ba câu khác nhau và không được lẫn: `bitrate_*` là **trần** (đặt EIT 400 kbps không làm nó chạy 400 kbps); `lap_*` mới **quyết định** bảng ra sóng bao lâu một lần; `tong` là tốc độ cả dòng, phần dư thành gói nhồi. Nhịp lặp bị chặn theo trần ETSI TS 101 211 §4.1 — NIT 10 s, SDT actual **2 s**, SDT other 10 s, BAT 10 s — và đó là phép kiểm **từ chối lưu**, không phải chú thích. Sai thì **không ghi**, như trang Đầu ra: `vtcsi run` đọc file này lúc khởi động nên một con số hỏng nằm sẵn là quả mìn cho lần dựng lại kế tiếp. **Khác `dau-ra.yaml`, file này VÀO git**: địa chỉ ra bắt buộc khác nhau giữa hai máy, còn nhịp lặp bắt buộc giống nhau — lệch thì hôm chuyển máy đầu thu thấy nhịp bảng đổi. Thiếu file thì dùng mặc định đúng bộ số đang phát, nên hệ chưa từng mở trang này vẫn phát y như trước. Đổi số xong phải **dựng lại dịch vụ phát**: dòng lệnh `tsp` chỉ được dựng một lần lúc khởi động. |
| **FR-102** | **Nút đặt lại bộ khuyến nghị, và dấu hiệu ở từng ô đang lệch.** Mặc định và khuyến nghị là **một** bộ, không phải hai: một bộ "mặc định" khác bộ "khuyến nghị" nghĩa là hệ tự khởi động bằng thứ chính nó bảo đừng dùng. Trang đánh dấu từng mục lệch kèm giá trị khuyến nghị của mục đó, nên bấm nút là biết trước mình đổi cái gì. Có hỏi lại, và hộp hỏi nói rõ **bao nhiêu mục** sẽ bị ghi đè. Đặt lại xong vẫn phải dựng lại dịch vụ phát, và thông báo mang nguyên câu lệnh. |
...
| **RO-13** | **File mẫu chỉ có một ngày dữ liệu** (2026-09-08), trong khi cửa sổ khai báo là hai ngày và yêu cầu độ sâu là tám ngày. | **Đã trả lời, 2026-09-15.** File thật mang **nhiều ngày** — đợt giao ngày 15/9 chứa 13, 14 và 15/9 — và mỗi `<SERVICE>` tự khai khoảng nó phụ trách, nên tích luỹ vẫn cần nhưng quy tắc đè là FR-95 chứ không phải cộng dồn. Độ sâu thực tế vẫn chỉ **một ngày về phía trước**: đợt giao dừng ở 16:59 hôm đó, và Barrowa đọc cùng file nên cũng dừng ở đúng chỗ ấy. Muốn sâu hơn thì phải làm việc với phòng chương trình; không sửa được bằng code. |
